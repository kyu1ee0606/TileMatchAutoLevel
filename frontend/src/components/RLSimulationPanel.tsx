...
// 플레이테스트(GamePlayer/TileRenderer)와 동일한 이미지 규칙
...
// 레벨 미리보기: 캔버스 렌더 — 플레이테스트(TileRenderer)와 동일한 합성 순서
// (t0 배경 → 타입 이미지(craft/stack/key 특수 경로) → 속성 오버레이 → 화살표/아이콘)
// 홀수 레이어 0.5칸 오프셋, 아래 레이어부터 그려 위 레이어가 덮는 인게임 스택 표현
async function renderLevelCanvasPreview(lv: LevelJSON, size = 420): Promise<string | null> {
  ...
  for (const t of tiles) {
    ...
    // 3) 열쇠 타일 아이콘 (70% 크기 중앙 — TileRenderer와 동일)
    if (t.type === 'key') {
    ...
    // 4) 속성 오버레이 (opacity 0.9 — TileRenderer와 동일)
    if (t.attr) {
      ...
      if (attrImg) {
        ...
        // 오버레이 이미지가 없는 기믹(curtain/teleport 등)은 배지로 표시 — 플레이테스트와 동일
    ...
    // 5) craft/stack/link 방향 화살표 (TileRenderer와 동일한 표기)

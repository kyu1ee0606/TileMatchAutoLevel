...
**마지막 업데이트**: 2026-03-10
**프로젝트 버전**: MVP + 레벨 생성 자동화 + UX 개선 + 봇 시뮬레이터 v15.10
...
### ✅ 패턴 모드 3배수 보정 개선 (v15.9 → v15.10)

**문제**: 패턴 모드에서 3의 배수를 맞추기 위해 타일을 삭제하면 패턴 모양 손상

**해결**: 우선순위 기반 3배수 보정 전략

| 우선순위 | 방법 | 결과 |
|----------|------|------|
| 1순위 | Goal(craft/stack) 내부 타일 수 조정 | 패턴 모양 100% 유지 |
| 2순위 | 인접 위치에 타일 추가 (Fallback) | 최소 변형 |

- **v15.9**: 패턴 모드에서 타일 삭제 방지, 인접 위치 추가 방식
- **v15.10**: Goal 내부 조정 우선, Fallback으로 인접 추가
- 관련 함수: `_ensure_tile_count_divisible_by_3`, `_force_fix_tile_counts`

### ✅ 패턴 템플릿 크기 조정 (v15.8)

- 삼각형 패턴 (30, 31, 36, 37): 8x8 그리드 전체 채움
- 번개/지그재그 패턴 (38): 8x8 그리드 전체 채움
- 쉐브론 패턴 (14): 두께 증가 (1px → 2-3px)
